...
##### Docker Desktopでアカウントサインインしてるとエラーが出る可能性があります。ログアウト推奨！
...
### TailwindCSSのビルドコマンドを手動で実行する
コンテナに入ったら、新しくターミナルをもう1つ開いて、以下のコマンドを実行してください。tailwindターミナルは消さないで！
```bash
tailwindcss -i ./app/assets/stylesheets/application.tailwind.css -o ./app/assets/stylesheets/tailwind.css --watch --poll
```
これでTailwindCSSが自動ビルド＆監視されるようになります！

※ --poll をつけているので、Docker環境でもちゃんとファイル変更が検知されます。


### ② Railsサーバーは自動起動しています！
Dockerコンテナを起動した時点で、
Railsサーバー（Puma）が ポート3000 で自動的に立ち上がる設定になっています。

👉 なので rails s コマンドなどを打つ必要はありません！
...
### 🔥 注意ポイント
コンテナ再起動後は、ターミナルで再度 **TailwindCSSビルドコマンド** を手動実行してください。tailwindターミナルは消さないで！
```bash
tailwindcss -i ./app/assets/stylesheets/application.tailwind.css -o ./app/assets/stylesheets/tailwind.css --watch --poll
```
